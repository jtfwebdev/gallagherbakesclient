...
import { faPlus, faMinus, faTrashCan, faAnglesLeft } from '@fortawesome/free-solid-svg-icons';
...
const Cart = ({setCartModalOpen, products, setBasket, basketTotal, setBasketTotal}: {setCartModalOpen: React.Dispatch<React.SetStateAction<boolean>>}) => {
    ...
    useEffect(() => {
        ...
        const handleClick = (event) => {
            ...
            if (screenWidth > 800 && cartPanelRef.current && !cartPanelRef.current.contains(event.target)) {
        ...
        >
            ...
            max-[900px]:w-[100%] max-[900px]:min-h-[95%] max-[900px]:rounded-none max-[900px]:px-2">
                <div className="flex justify-between mb-4">
                    <h2 className="font-text text-secondary-100 text-2xl border-b-2 border-secondary-100 w-fit">{screenWidth > 440 ? "Your shopping cart" : "Shopping cart"}</h2>
                    <div className="flex items-center gap-2" onClick={() => setCartModalOpen(false)}>
                        <FontAwesomeIcon icon={faAnglesLeft} className="text-3xl text-secondary-100" />
                        <span className="font-text text-secondary-100 text-xl">{screenWidth > 440 ? "Close cart" : "Close"}</span>
                    </div>
                </div>
                
                <div className="h-full">
                    {basket && basket[0] && <div className="flex flex-col h-full gap-4">
                        {screenWidth > 500 ? basket.map((product, idx) => {
                        ...
                        basket.map((product, idx) => {
                            return <AnimatePresence><MobileCartProduct key={idx} products={products} product={product} basket={basket} setBasket={setBasket} /></AnimatePresence>
                        })}
...
const CartProduct = ({products, product, basket, setBasket}) => {
    ...
    return (
            <motion.div initial={{opacity: 0, y: -200}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: -200}} className="individual_cart_product flex border-b-2 border-secondary-100 border-opacity-10 pb-4">
                <img className="w-44 max-[600px]:w-32" src={productDetails[0].images[0].src} alt="" />
                <div className="pl-4 flex flex-1 items-center justify-between">
                    ...
                    <div className="flex items-center justify-center max-[600px]:flex-col max-[600px]:ml-2">
                        ...
                        <div className="text-red-400 hover:text-red-200 hover:cursor-pointer duration-100 max-[600px]:mt-2" onClick={handleDeleteFromCart}>
...
const MobileCartProduct = ({products, product, basket, setBasket}) => {
    ...
    return (
        <motion.div initial={{opacity: 0, y: -200}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: -200}} className="individual_cart_product flex border-b-2 border-secondary-100 border-opacity-10 pb-4">
            <div className="flex flex-col items-center justify-center w-fit">
                <img className="w-44 max-[600px]:w-28" src={productDetails[0].images[0].src} alt="" />
            ...
            <div className="pl-4 flex flex-1 items-center justify-between">
                ...
                <div className="flex items-center justify-center max-[600px]:flex-col max-[600px]:ml-2">
                    <div className="flex items-center mr-4 gap-4 max-[375px]:gap-2">
                        <FontAwesomeIcon icon={faMinus} className="text-secondary-100 text-2xl hover:opacity-60 hover:cursor-pointer duration-100 max-[375px]:text-lg" onClick={handleDecrement} />
                        <input type="number" value={quantity} className="bg-gray-50 border border-gray-300 w-12 text-center text-gray-900 sm:text-sm rounded-lg focus:ring-secondary-100 focus:border-secondary-100 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" onChange={(e) => setQuantity(Number(e.target.value))}/>
                        <FontAwesomeIcon icon={faPlus} className="text-secondary-100 text-2xl hover:opacity-60 hover:cursor-pointer duration-100 max-[375px]:text-lg" onClick={() => setQuantity((prev) => prev + 1)} />
                    </div>
                    <div className="text-red-400 hover:text-red-200 hover:cursor-pointer duration-100 max-[600px]:mt-2" onClick={handleDeleteFromCart}>
                        Remove <FontAwesomeIcon icon={faTrashCan} className="text-inherit" />
                    </div>
