    ...
    return ( 
        <>
            ...
            max-[530px]:w-[100%] max-[530px]:px-0 max-[530px]:py-0 max-[530px]:my-0">
                {product && screenWidth > 768 ? <FullScreenProduct product={product} buttonText={buttonText} handleAddToBasket={handleAddToBasket} quantity={quantity} handleDecrement={handleDecrement} setQuantity={setQuantity} />
                : <MobileProduct product={product} buttonText={buttonText} handleAddToBasket={handleAddToBasket} quantity={quantity} handleDecrement={handleDecrement} setQuantity={setQuantity} />}
            </div>
            <Divider />
        </>
     );
...
const FullScreenProduct = ({product, buttonText, handleAddToBasket, quantity, handleDecrement, setQuantity}) => {
    return (
...
const MobileProduct = ({product, buttonText, handleAddToBasket, quantity, handleDecrement, setQuantity}) => {
    return (
        <div className="flex flex-col h-fit justify-center mx-auto max-w-full max-[530px]:pb-4">
            {product && <>
            <div className="h-fit">
                <img src={product[0].images[0].src} alt="" className="w-[75vw] max-[530px]:w-full" />
            </div>
            <div className="mt-2 font-text text-secondary-100 flex flex-col flex-1 w-[75vw] max-[530px]:w-full max-[530px]:px-4">
                <h2 className="text-4xl pb-2 font-bold border-b-2 border-secondary-100 max-w-fit text-wrap">{product[0].name}</h2>
                <div className="mt-4" dangerouslySetInnerHTML={{__html: product[0].description}}></div>
            </div>
            <form className="flex items-center mt-4 max-w-fit max-[530px]:flex-col max-[530px]:items-start max-[530px]:px-4" onSubmit={handleAddToBasket}>
                <div className="flex items-center mr-4 gap-4">
                    <FontAwesomeIcon icon={faMinus} className="text-secondary-100 text-2xl hover:cursor-pointer hover:opacity-50 duration-150" onClick={handleDecrement} />
                    <input type="number" value={quantity} className="bg-gray-50 border border-gray-300 w-12 text-center text-gray-900 sm:text-sm rounded-lg focus:ring-secondary-100 focus:border-secondary-100 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" onChange={(e) => setQuantity(Number(e.target.value))}/>
                    <FontAwesomeIcon icon={faPlus} className="text-secondary-100 text-2xl hover:cursor-pointer hover:opacity-50 duration-150" onClick={() => setQuantity((prev) => prev + 1)} />
                </div>
                <button type="submit" className="px-4 py-2 rounded-full w-fit min-w-60 bg-secondary-100 font-text text-primary-100 text-2xl hover:opacity-80 hover:cursor-pointer duration-150 max-[530px]:mt-4">
                    <div className="select-none">
                        {buttonText}
                    </div>
                </button>
